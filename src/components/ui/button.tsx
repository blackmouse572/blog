...
export const Icon: React.FC<IconProps> = ({ className, children, size = 'md', type = 'leading' }) => {
  return <>{cloneElement(children as React.ReactElement, icon({ size, type, className: cn('', className) }))}</>;
};
...
export const Root = React.forwardRef<HTMLButtonElement & HTMLAnchorElement, ButtonProps>(
  (
    {
      className,
      intent = 'primary',
    ...
    const iconOnly =
      ...
      React.Children.toArray(children).some(
        (child) => React.isValidElement(child) && child.type === Icon && child.props.type === 'only'
      );
